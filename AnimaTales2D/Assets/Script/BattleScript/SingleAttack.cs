...
{
    private const string PlayerTurnPortrait = "Player Turn Portrait";
    private const string EnemyTurnPortrait = "Enemy Turn Portrait";
    IBattleManager bm;
    ...
    public IEnumerator SingleAllyAttack(AnimaActions anima , int selectEnemy)
    {
        ...
        if (bm.EnemyActions[selectEnemy].animaData.Animadie)
        {
            ...
            {
                bm.Turn[bm.TurnIndex].transform.Find(PlayerTurnPortrait).GetComponent<UnityEngine.UI.Image>().color = new Color(77f / 255f, 77f / 255f, 77f / 255f);
            }
            else
            {
                bm.Turn[bm.TurnIndex++].transform.Find(PlayerTurnPortrait).GetComponent<UnityEngine.UI.Image>().color = new Color(77f / 255f, 77f / 255f, 77f / 255f);
    ...
    public IEnumerator SingleEnemyAttack(EnemyActions enemy, int selectAlly)
    {
        ...
        if (bm.AllyActions[selectAlly].animaData.Animadie && aliveBeforeSkill.Contains(bm.AllyActions[selectAlly]))
        {
            ...
            {
                bm.Turn[bm.TurnIndex].transform.Find(EnemyTurnPortrait).GetComponent<UnityEngine.UI.Image>().color = new Color(77f / 255f, 77f / 255f, 77f / 255f);
            }
            else
            {
                bm.Turn[bm.TurnIndex++].transform.Find(EnemyTurnPortrait).GetComponent<UnityEngine.UI.Image>().color = new Color(77f / 255f, 77f / 255f, 77f / 255f);
    ...
    public IEnumerator SingleAllyShield(AnimaActions anima, int selectAlly, int skillNum, float weight)
    {
        ...
        bm.BattleLogManager.AddLog($"{anima.animaData.Name}이 \"{anima.animaData.skillName[skillNum]}\"를 사용해 {bm.AllyActions[selectAlly].animaData.Name} 에게 {Mathf.FloorToInt(anima.heal)}베리어 씀", true);
        bm.Turn[bm.TurnIndex++].transform.Find(PlayerTurnPortrait).GetComponent<UnityEngine.UI.Image>().color = new Color(77f / 255f, 77f / 255f, 77f / 255f);
        BuffUpdate(anima.animaData);
    }
    public IEnumerator SingleEnemyShield(EnemyActions enemy, int selectEnemy, float weight)
    {
        ...
        bm.BattleLogManager.AddLog($"{enemy.animaData.Name}이 \"{enemy.animaData.skillName[0]}\"를 사용해 {bm.EnemyActions[selectEnemy].animaData.Name} 에게{Mathf.FloorToInt(enemy.heal)}베리어 씀", false);
        bm.Turn[bm.TurnIndex++].transform.Find(EnemyTurnPortrait).GetComponent<UnityEngine.UI.Image>().color = new Color(77f / 255f, 77f / 255f, 77f / 255f);
        BuffUpdate(enemy.animaData);
    }
    ...
    public IEnumerator FelixRoundSkill(EnemyActions enemy, int selectAlly)
    {
        ...
        bm.BattleLogManager.AddLog($"{enemy.animaData.Name} used \"라운드 스킬\" on {bm.AllyActions[selectAlly].animaData.Name} for {Mathf.FloorToInt(enemy.damage)} damage", false);
        if (bm.AllyActions[selectAlly].animaData.Animadie && aliveBeforeSkill.Contains(bm.AllyActions[selectAlly]))
        {
            if (DefeatAlly(bm.AllyActions[selectAlly], selectAlly))
            {
    ...
    public IEnumerator PhobiaRoundSkill(EnemyActions enemy, List<AnimaDataSO> turnList, int selectAlly)
    {
        ...
        foreach(var anima in turnList)
        {
            ...
            {
                bm.Turn[turnList.IndexOf(anima)].transform.Find(PlayerTurnPortrait).GetComponent<UnityEngine.UI.Image>().color = new Color(0f / 255f, 0f / 255f, 0f / 255f);
            }
        }
        bm.BattleLogManager.AddLog($"\"라운드스킬\"로 {bm.AllyActions[selectAlly].animaData.Name}은 턴 봉인됨", false);
    }
