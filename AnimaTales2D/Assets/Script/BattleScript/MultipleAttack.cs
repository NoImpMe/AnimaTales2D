...
public class MultipleAttack : MonoBehaviour
{
    ...
    public IEnumerator MultiAllyShield(AnimaActions anima, int skillNum, float weight)
    {
        ...
        bm.BattleLogManager.AddLog($"{anima.animaData.Name}이 \"{anima.animaData.skillName[skillNum]}\"를 사용해 아군에게 {Mathf.FloorToInt(anima.maxHeal)}배리어 사용", true);
        bm.Turn[bm.TurnIndex++].transform.Find(PlayerTurnPortrait).GetComponent<UnityEngine.UI.Image>().color = new Color(77f / 255f, 77f / 255f, 77f / 255f);
        BuffUpdate(anima.animaData);
    }
    ...
    public IEnumerator IrascorRoundSkill(EnemyActions enemy)
    {
        ...
        bm.BattleLogManager.AddLog($"\"라운드 스킬\" 으로 아니마들이 for {Mathf.FloorToInt(enemy.damage)} 데미지 입음", false);
        List<int> allyList = new List<int>();
    ...
    public IEnumerator HavetRoundSkill(EnemyActions enemy)
    {
        ...
        bm.BattleLogManager.AddLog($"{enemy.animaData.Name}가 \"라운드스킬\"을 사용해 {enemy.downGold}골드를 도둑맞았다 ", false);
        List<int> allyList = new List<int>();
