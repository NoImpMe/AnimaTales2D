...
public class ShopManager : MonoBehaviour
{
    ...
    private void RefreshShop()
    {
        ...
        var allItems = shopList.GetAllItems();
        foreach (var item in allItems)
        {
            remainingCounts[item.itemID] = shopState.GetRemainingCount(item.itemID);
        }

        ShopUIManager.Instance.ShowShopItems(allItems, remainingCounts, OnItemPurchase);
    }
    ...
    private IEnumerator ProcessItemPurchase(ShopItemData item, System.Action<bool> callback)
    {
        ...
        yield return StartCoroutine(SuccessPurchase(item));
        callback?.Invoke(true);
    }

    private IEnumerator SuccessPurchase(ShopItemData item)
    {
