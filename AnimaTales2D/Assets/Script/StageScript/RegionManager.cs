...
public class RegionManager : MonoBehaviour
{
    ...
    public void StageInit(int stageNum)
    {
        ...
        PlayBgmForRegionType(tile.transform.parent.name);
        SetNextTile(tile);
        GameObject.Find("DontDesManager").GetComponent<DontDesManager>().setDesGrid();
    }

    private void PlayBgmForRegionType(string regionTypeName)
    {
        for (int i = 0; i < tileType.Count; i++)
        {
            if (regionTypeName == tileType[i])
            {
    ...
    private void DimAndDisableTile(RegionController target)
    {
        target.gameObject.GetComponent<TilemapCollider2D>().enabled = false;

        var tilemap = target.gameObject.GetComponent<Tilemap>();
        var targetColor = tilemap.color;
        targetColor.a = 0.37f;
    ...
    public IEnumerator EnterBattle(RegionController target)
    {
        ...
        else if (target.name.StartsWith("Start"))
        {
            ...
            target.GetComponentInParent<IsVisitedField>().isVisited = true;
            PlayBgmForRegionType(target.transform.parent.name);
            SetNextTile(target);
        }
        else
        {
