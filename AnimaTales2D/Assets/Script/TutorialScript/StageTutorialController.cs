...
public class StageTutorialController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        GameObject villageTileObj = GameObject.Find("VillageTile");
        if (villageTileObj != null)
        {
            RegionController regionController = villageTileObj.GetComponent<RegionController>();
            if (regionController != null)
            {
                villageTile = regionController;
...
"저곳은 마을 스테이지라네. 쉬어갈 곳도 있고, 필요한 걸 챙기기도 좋지. 자네가 아마 가장 중요하게 생각할 곳이 될 것이네."
        ...
        DialogueSystem.Instance.StartDialogue(
            ...
            nextCondition: () => {
                ...
                if(idx == 4)
                {
                    GameObject animaInvPanel = GameObject.Find("Anima Inventory Panel");
                    if (animaInvPanel != null)
                    {
                        animaInvPanel.SetActive(false);
                    }
                }
                if(idx == 5)
                {
                    ...
                    invenBlock.SetActive(false);
                    GameObject animaInvPanel = GameObject.Find("Anima Inventory Panel");
                    if (animaInvPanel != null) animaInvPanel.SetActive(false);
