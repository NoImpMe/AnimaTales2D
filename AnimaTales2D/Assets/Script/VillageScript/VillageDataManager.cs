...
public class VillageDataManager : MonoBehaviour
{
    ...
    {
        if (villageShops.TryGetValue(villageID, out VillageShopState existingState))
        {
    ...
    {
        if (!villageShops.TryGetValue(villageID, out VillageShopState state))
        {
            return false;
        }

        return state.PurchaseItem(itemID);
    }
